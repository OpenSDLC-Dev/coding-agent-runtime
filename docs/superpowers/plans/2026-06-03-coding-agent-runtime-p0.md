...
> `-p 127.0.0.1:8080:8080` 把宿主侧绑回环——未鉴权端点不暴露到局域网（安全加固 ④，见 Task 9）。容器内 `RUNTIME_HOSTNAME=0.0.0.0`（Dockerfile 设）让进程监听容器全网卡，docker 端口映射才能转发。

Expected: 日志出现 `[entrypoint] wrote /claude-config/CLAUDE.md`，随后 `runtime listening on http://0.0.0.0:8080`（容器内监听 0.0.0.0；宿主只在 127.0.0.1:8080 可达）。
...
## Task 9: P0 安全兜底（提前 P3 的低成本项）

> 来源：三轮 commit 安全审查（runtime.ts / server.ts / index.ts）。经与用户确认，把 4 个低成本、不破坏「无人值守 + 容器=边界」设计的项提前进 P0；`model` 白名单（影响小、需新配置项）留 P1。**保持锁定不变**：无 HTTP 鉴权（可信网络）、`bypassPermissions`（容器=边界）、`env` 整体 spread（SDK 必需）。

**Files:**
- Modify: `apps/runtime/src/agent/runtime.ts`（① disallowedTools 兜底）
- Modify: `apps/runtime/src/server.ts`（③ 通用错误 + correlationId）
- Modify: `apps/runtime/src/agent/config.ts` + `src/index.ts` + `Dockerfile` + `.env.example`（⑤ RUNTIME_HOSTNAME）
- Modify: 对应 test（helpers/config/runtime/server）

- [ ] **① `disallowedTools` 兜底 deny**（`runtime.ts` 的 `query()` 选项内，紧随 `allowDangerouslySkipPermissions`）
...
测试（`runtime.test.ts`）：用捕获式 `queryFn` 断言传入 `query()` 的 options 含 `permissionMode:"bypassPermissions"`、`allowDangerouslySkipPermissions:true`、`disallowedTools` 含上述 3 项。

- [ ] **③ 通用错误 + 服务端日志 + correlationId**（`server.ts`）

顶部加 `import { randomUUID } from "node:crypto";`；SSE catch 改为：
...
测试（`server.test.ts`）：注入「调用即抛错」的 `queryFn`，断言流里有 `event: error`、含 `internal error` 与 `correlationId`、**不含**原始错误串。

- [ ] **④ docker 绑回环**：Task 8 的 `docker run` 用 `-p 127.0.0.1:8080:8080`（已更新）；spec §8.2 同步。

- [ ] **⑤ `RUNTIME_HOSTNAME` 可配，默认 `127.0.0.1`**

`config.ts`：`RuntimeConfig` 加 `hostname: string`；`loadConfig` 加 `hostname: env.RUNTIME_HOSTNAME || "127.0.0.1"`。
`index.ts`：`serve({ ..., hostname: config.hostname })`，日志用 `config.hostname`。
`Dockerfile` 运行时 ENV 加 `RUNTIME_HOSTNAME=0.0.0.0`（容器需监听全网卡供 `-p` 转发）。
`.env.example` 加 `RUNTIME_HOSTNAME=127.0.0.1`（带说明）。
测试：`helpers.ts` testConfig 加 `hostname`；`config.test.ts` 解析用例加 `RUNTIME_HOSTNAME` 入参与断言、默认用例断言 `hostname === "127.0.0.1"`。

- [ ] **验证 + 提交**

Run: `pnpm --filter @app/runtime test`（12 passed）、`typecheck`（0）、`pnpm exec biome check .`（0）。
Run（efficacy，需真实密钥）：Task 8 里追加一轮让 agent 尝试 `curl`，确认被 deny（`permission_denials` 或工具被拒）。

```bash
git add apps/runtime/src apps/runtime/test apps/runtime/Dockerfile .env.example docs/
git commit -m "feat(security): P0 兜底 — disallowedTools/通用错误/RUNTIME_HOSTNAME(默认回环)"
```
...
- [ ] `pnpm --filter @app/runtime test` 全绿（config + runtime + server，含 Task 9 安全用例，共 12）。
- [ ] `pnpm --filter @app/runtime typecheck` 与 `pnpm run check` 均退出码 0。
- [ ] `docker build` 成功。
- [ ] `docker run`（`-p 127.0.0.1:8080:8080`）后 `POST /sessions` 流式返回 agent 输出，`/claude-config/CLAUDE.md` 为固定 guidelines，改动落在挂载 `/workspace`（Task 8 验收）。
- [ ] 安全兜底生效：agent 的 `curl/wget/sudo` 尝试被 deny（Task 9 efficacy，Task 8 中实测）。
