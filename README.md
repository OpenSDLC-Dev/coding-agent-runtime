...
- **Multimodal image input** (opt-in): a turn request may carry a `content` array of `text` and base64 `image` blocks instead of a plain `prompt` (exactly one of the two). When `content` is present the runtime uses the SDK's streaming-input mode; a plain `prompt` keeps the unchanged single-message path. Input is size-bounded — a request-body limit (`RUNTIME_MAX_BODY_BYTES`, default 12 MiB) returns `413` before buffering, and per-content limits (≤16 images, ≤10 MiB base64) return `400`. The web playground composer supports image attach (click or drag-and-drop). Interactive approvals (`canUseTool` / `AskUserQuestion`) are out of scope — they need a duplex transport the unidirectional SSE stream can't provide.
