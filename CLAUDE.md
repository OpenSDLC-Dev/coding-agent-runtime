...
**Extensibility:** operator-defined custom tools / external MCP servers / hooks / skills plug in through a self-contained `apps/runtime/src/extensions/` subsystem with two authoring tiers — compiled-in code extensions (`BUILTIN_EXTENSIONS`, for in-process tools/hook callbacks) and a declarative JSON manifest (`RUNTIME_EXTENSIONS_FILE`, for external MCP / plugins / skills / dirs). All contributions are folded by `loadExtensions` (once at startup) and merged by one pure composer (`applyExtensions`) that layers a hand-written **safe subset** of `Options` onto the secure base; `runTurn` behaves exactly as before when no extensions are configured.

**Status:** phases P0–P3 complete, plus hosting production guards and a CHANGELOG. Current release is **v0.7.0** (delivery-workflow tooling — `pnpm verify`, `scripts/smoke.mjs`, the `coding-agent-runtime-*` skill set, and `CONTRIBUTING.md` — plus the Agent SDK and decoupled Claude Code CLI bumped to latest, `/loop`–`/goal` slash-command pass-through tests, and an esbuild security pin) — Git-tagged with a GitHub Release titled `coding-agent-runtime v0.7.0`, every `package.json` at `0.7.0`. The prior release was v0.6.0 (hosting production guards). See `CHANGELOG.md` for the full history.
