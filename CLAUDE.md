...
**Benchmarking:** an evaluation/baseline subsystem (in progress) for objectively measuring the runtime's coding ability against industry-standard benchmarks. It lives in `apps/bench` (`@app/bench`) as an **external orchestrator** — it drives the runtime as a black box over the public HTTP/SSE contract and scores out-of-band, so it is deliberately *not* an extension and never touches the runtime core or its security perimeter. The phased plan is SWE-bench-flagship-first. **E0** (done) is the walking skeleton — the harness pipeline (prepare workspace → drive a turn → pluggable `Scorer` → versioned zod `RunReport`) proven on the in-repo `hello-bench`. **E1** (done) adds the **SWE-bench adapter** (`--benchmark swe-bench`, SWE-bench Lite curated id-list) with authoritative **batch** scoring: a turn that completes yields a `git diff` prediction against a pinned base ref (a new `prepare()` workspace seam checks out each repo at its `base_commit` via a shallow fetch by SHA; the runner strips that `.git` after every turn so the next reset's repo-guard stays clean), then the whole run is scored once by the official local `swebench` Docker harness, whose `schema_version: 2` per-instance ids fold back into the `RunReport`. The scorer is pluggable (`BatchScorer`); E1 ships the source-verified **offline Docker** backend (the hosted `sb-cli` backend is a planned follow-up, gated on confirming its report exposes per-instance ids). The repo commits only the curated id-list — never copyrighted issue text / gold patches; full records come from an operator-downloaded dataset file. **E2** (done) adds **baseline + regression tracking**: every run snapshots the **config tuple** that produced it (benchmark / subset hash / operator `--backend-label` / model / effort + max-turns from `/config` / prompt-scaffold version) and embeds it in the `RunReport` (schema v2). The tuple's identity subset (excluding runtime/harness versions, so a newer runtime is measured against the same bar) hashes to a **baseline key**; the CLI gains `--accept` (commit a baseline — the only way one is written), `--compare` (non-zero exit on a resolve-rate drop = the regression gate), `--update-history` (per-key JSONL trend trail), and an `emit-markdown` command that regenerates the committed `BENCHMARKS.md` — all offline and unit-tested with synthetic reports. The one runtime-core touch was additive: `GET /config` now also exposes `effort` + `maxTurns` so the harness snapshots them authoritatively. Wiring benchmark runs into scheduled / PR-gated CI is deliberately deferred until real runs are routine. Later milestones add Aider Polyglot. Operator guide: `docs/benchmarks.md`. Run with `node scripts/bench.mjs`; the harness's vitest suite (fake runtime + recorded SSE fixtures + synthetic fixtures + stub/fake scorers) runs in `pnpm verify` with no model, Docker, or network.

**Status:** phases P0–P3 complete, plus hosting production guards, a CHANGELOG, the pluggable extension subsystem, and the benchmark/evaluation subsystem. Current release is **v0.9.0** (benchmark & evaluation subsystem — the `apps/bench` external orchestrator E0+E1+E2 with the additive `GET /config` exposure of `effort` + `maxTurns` — plus two SSE turn-contract fixes: the `tool_result` event now carries the tool's actual `content`, and the turn-submission endpoints gain post-turn idempotency via an always-on per-session in-flight lock + an opt-in `Idempotency-Key`) — Git-tagged with a GitHub Release titled `coding-agent-runtime v0.9.0`, every `package.json` at `0.9.0`. The prior release was v0.8.0 (pluggable extension subsystem). `[Unreleased]` carries four Agent-SDK-alignment changes: **multimodal image input** (a per-request `content` array of text/base64-image blocks drives the SDK's streaming-input mode — exactly one of `prompt`/`content`, bounded and `400`-guarded; the `apps/web` composer gains image attach/thumbnails; interactive approvals `canUseTool`/`AskUserQuestion` stay out of scope as SSE-incompatible), opt-in **structured outputs** (a per-request `outputFormat` JSON-schema envelope is forwarded to the SDK and the validated `structured_output` is surfaced on the SSE `result` event), a `hono` `4.12.25` security patch (CORS wildcard advisory GHSA-88fw-hqm2-52qc), and a streaming-output fix — token-level `delta` SSE events are now surfaced from the SDK's partial messages (gated by `INCLUDE_PARTIAL_MESSAGES`, which previously had no effect because the SDK→SSE mapper dropped `stream_event` frames). See `CHANGELOG.md` for the full history.
