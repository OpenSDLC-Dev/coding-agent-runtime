...
- `GET /config` now also reports `effort` (the runtime's reasoning-effort level) and `maxTurns` (the turn backstop). Both are operational, non-secret settings already configured via `RUNTIME_EFFORT` / `RUNTIME_MAX_TURNS`; exposing them lets the external benchmark harness authoritatively snapshot a run's config tuple (the next milestone's baseline key) instead of guessing. The model-backend URL remains deliberately unexposed. Additive and backward-compatible — existing `/config` consumers ignore the new fields.
