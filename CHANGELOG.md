...
- Upgrade the Claude Agent SDK (`@anthropic-ai/claude-agent-sdk` `0.3.177` → `0.3.197`) and the decoupled standalone Claude Code CLI (`CLAUDE_CODE_VERSION` `2.1.177` → `2.1.197` in the Dockerfile) to their latest releases, keeping the deliberately version-pinned SDK × CLI pair in lockstep. The only upstream breaking change in this window — the CLI's removal of the Agent Teams `TeamCreate`/`TeamDelete` tools (claude-code `2.1.178`) — is not used by the runtime, and the SDK surface the runtime depends on (`query`, `Options`, `SDKMessage`/`SDKUserMessage`, `EffortLevel`, `HookEvent`, and the `getSessionInfo`/`getSessionMessages`/`deleteSession` session helpers) is unchanged and typechecks clean against the new types. The CI `smoke` job's one real turn (against the MiniMax-M3 Anthropic-compatible backend) covers the end-to-end upgrade.
...
- SWE-bench adapter + authoritative offline scoring for the harness (`apps/bench/src/adapters/swebench/`, `--benchmark swe-bench`). It introduces a batch-scoring path the flagship benchmark genuinely needs: SWE-bench cannot be scored one instance at a time, so the runner now collects a `git diff` **prediction** per completed turn (against a pinned base ref, so a fix the agent commits is still captured; a new optional `prepare()` workspace seam checks out each repo at its `base_commit` via a shallow fetch by SHA, and the runner removes that `.git` after every turn so the next reset's repo-guard never trips), then scores the whole run **once** with the official local harness (`python -m swebench.harness.run_evaluation`) and folds the exact per-instance verdicts (`resolved_ids`/`unresolved_ids`/`error_ids` from its `schema_version: 2` report) back into the `RunReport`. The repo commits only a curated **instance-id list** (`apps/bench/subsets/swebench-lite-curated.json`, 5 verified SWE-bench Lite ids across 5 repos) — never copyrighted issue text or gold patches; the full records come from an operator-downloaded dataset file at run time. Scoring backend is pluggable (`BatchScorer`); E1 ships the source-verified Docker scorer, whose report parsing is unit-tested against synthetic fixtures (zero model / zero Docker / zero network in CI). The hosted `sb-cli` scorer is a planned follow-up (it needs one real submission to confirm its report exposes per-instance ids). Operator guide: `docs/benchmarks.md`. Config-tuple baseline and regression tracking land in E2 (below).
- Baseline + regression tracking for the benchmark harness (E2). Every run now snapshots the **config tuple** that produced it (benchmark, subset hash, operator `--backend-label`, model, effort, max-turns from the runtime's `/config`, and a prompt-scaffold version) and embeds it in the `RunReport` (schema bumped to v2) so the artifact is self-describing. The identity subset of that tuple — deliberately excluding the runtime/harness versions, so a newer runtime is measured against the same bar — hashes to a **baseline key**. New CLI surface (all offline, on both benchmarks): `--accept` commits a run as the committed baseline (`apps/bench/baselines/<key>.json`, written only by this explicit human action), `--compare` exits non-zero when a run's resolve rate drops below the committed baseline (the regression gate), `--update-history` appends a JSON line to a per-key trend trail (`apps/bench/history/<key>.jsonl`), and an `emit-markdown` command regenerates the committed `BENCHMARKS.md` from the baselines (no runtime needed). Pure functions throughout (`config-snapshot`, `baseline`, `compare`, `history`, `report/markdown`), unit-tested with synthetic reports — zero model / Docker / network. Wiring this into scheduled or PR-gated CI is deliberately deferred until real runs are routine. Operator guide: `docs/benchmarks.md`.
- Post-turn idempotency for the turn-submission endpoints (`POST /sessions`, `POST /sessions/:id/turns`). Two complementary, source-verified guards stop a retried or duplicated submission from running a second turn on the same session — which would orphan the in-flight `AbortController` and run two CLIs against the one `/workspace`. (1) An always-on **per-session in-flight lock**: a new `SessionRegistry.tryReserve` atomically claims the session's active slot, so a continuation turn for a session that already has one in flight is rejected with HTTP `409` instead of clobbering it. (2) An opt-in **`Idempotency-Key`** header (new `IdempotencyStore`, retention window `RUNTIME_IDEMPOTENCY_TTL_MS`, default 10m, `0` disables): a duplicate key — in-flight, or completed within the window — is rejected with `409` (the response carries the original `sessionId`), giving clients at-most-once submission across retries even on the create path (where no session id exists yet to lock on). Both are independent of the cross-session `RUNTIME_MAX_CONCURRENT_TURNS` `429` admission limit. Unit-tested with synthetic fixtures and a fake clock (zero model / network / Docker); the new `409` and `Idempotency-Key` header are documented in the OpenAPI spec, README, and `.env.example`.
